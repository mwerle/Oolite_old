...
ifeq ($(GNUSTEP_HOST_OS),mingw32)
	...
	ADDITIONAL_OBJC_LIBS = -lglu32 -lopengl32 -lmingw32 -lSDLmain -lSDL -lSDL_mixer -lSDL_image -lSDL_ttf -lgnustep-base
	ADDITIONAL_CFLAGS = -DLINUX -DWIN32 -DNEED_STRLCPY `sdl-config --cflags`
# note the vpath stuff above isn't working for me, so adding src/SDL and src/Core explicitly
	ADDITIONAL_OBJCFLAGS = -DLOADSAVEGUI -DLINUX -DWIN32 -DHAVE_SOUND -DNEWFONTS -Isrc/SDL -Isrc/Core -Wno-import `sdl-config --cflags`
