...
int max_planet_textures = 10;
NSMutableArray* planet_texture_mru_cache = nil;
#endif
...
+ (GLuint) getTextureNameForPlanet:(struct planet_info*)info
{
	...
	{
		NSLog(@"d");
		texName = (GLuint)[(NSNumber *)[[textureUniversalDictionary objectForKey:filename] objectForKey:@"texName"] intValue];
		NSLog(@"e");
	}

	NSLog(@"f");
	[TextureStore updatePlanetTextureCacheWith:filename];
	NSLog(@"g");
	return texName;
}
...
{
// This code not working in latest version - something to do with the change to a static method? The good news is this cache wasn't actually used.
/*
	if (planet_texture_mru_cache == nil)
	{
		NSLog(@"aa");
		planet_texture_mru_cache = [NSMutableArray arrayWithCapacity:max_planet_textures];
		NSLog(@"ab");
	}
	...
	if (cacheEntryIdx == NSNotFound)
	{
		...
		if (cacheSize > max_planet_textures)
		{
			...
			GLuint texName = (GLuint)[(NSNumber *)[[textureUniversalDictionary objectForKey:dropOff] objectForKey:@"texName"] intValue];
			glBindTexture(GL_TEXTURE_2D, 0);
			NSLog(@"deleting texture #%d (%@)", texName, dropOff);
			glDeleteTextures(1, &texName);
			NSLog(@"texture deleted, removing from textureDictionary");
			[textureUniversalDictionary removeObjectForKey:dropOff];
		}
	}
	else
	{
